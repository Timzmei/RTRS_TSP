...
from route import find_nearest_point, calculate_route_length
...
def ant_colony_optimization(distance_matrix, num_ants, num_iterations, pheromone_evaporation, pheromone_constant, alpha, beta):
    ...
    def run_colony(seed):
        ...
        for iteration in range(num_iterations):
            ...
            global_ant_routes.append(best_route)
            global_best_route = min(global_ant_routes, key=lambda route: sum(distance_matrix[i][j] for i, j in zip(route, route[1:])))
            global_bst_route_length = sum(distance_matrix[i][j] for i, j in zip(global_best_route, global_best_route[1:]))
        ...
        ant_route = [x+1 for x in global_best_route]
    ...
    with Pool(num_colonies) as pool:
        ...
        best_routes = pool.map(run_colony, seeds)
    
    # Находим лучший маршрут среди всех колоний
    global_best_route = min(best_routes, key=lambda route: sum(distance_matrix[i][j] for i, j in zip(route, route[1:])))
